...
class VideoAnchorNode : AnchorNode() {
    ...
    private fun scaleNode(
        ...
        videoNode.localScale = when (videoScaleType) {
            FIT_XY -> scaleFitXY(imageWidth, imageHeight)
            CENTER_CROP -> scaleCenterCrop(videoWidth, videoHeight, imageWidth, imageHeight)
            CENTER_INSIDE -> scaleCenterInside(videoWidth, videoHeight, imageWidth, imageHeight)
        }
    }
